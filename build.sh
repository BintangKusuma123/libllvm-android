...
case "$1" in
    ...
    armv7-a)
        ...
        CMAKEOPTIONS=-DCMAKE_C_FLAGS=$CFLAGS -DCMAKE_CXX_FLAGS=$CFLAGS -DLLVM_TABLEGEN=/usr/bin/llvm-tblgen -DCLANG_TABLEGEN=$PWD/buildcmake/bin/clang-tblgen -DLLVM_ENABLE_PROJECTS=clang -DLLVM_ENABLE_TERMINFO=ON -DLLVM_ENABLE_THREADS=ON -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD="ARM" -DLIBCLANG_BUILD_STATIC=ON -DLLVM_INCLUDE_TESTS=OFF -DLLVM_ENABLE_ZLIB=ON -DCMAKE_CROSSCOMPILING=True -DLLVM_DEFAULT_TARGET_TRIPLE=armv7a-linux-androideabi -DLLVM_TARGET_ARCH=ARM -DLLVM_ENABLE_PIC=True -DCMAKE_TOOLCHAIN_FILE=$NDK/build/cmake/android.toolchain.cmake
